...
export const handler: Handler<APIGatewayProxyEvent, APIGatewayProxyResult> = async (event, context) => {
  try {
    ...
    if (projectsStatus !== 200) {
      console.error(`Error fetching projects: status=${projectsStatus}, error=${projects.$response.error}`);
      return Response(500, { message: "Could not fetch list of projects" });
    }
    if (aboutStatus !== 200) {
      console.error(`Error fetching about: status=${aboutStatus}, error=${about.$response.error}`);
